...
Parallel Value Iteration solver. It allows to benefits from multiple CPUs to speed up Value Iteration.
# Fields:
...
"""
mutable struct ParallelValueIterationMacroSolver <: Solver
...
function ParallelValueIterationMacroSolver(;max_iterations::Int64 = 100,
                                       belres::Float64 = 1e-3,
    ...
    return ParallelValueIterationMacroSolver(max_iterations, belres, n_procs, verbose, include_Q, init_util, state_order)
end

function solve(solver::ParallelValueIterationMacroSolver, mdp::Union{MDP,POMDP}; kwargs...)

    # deprecation warning - can be removed when Julia 1.0 is adopted
    if !isempty(kwargs)
        warn("Keyword args for solve(::ParallelValueIterationMacroSolver, ::MDP) are no longer supported. For verbose output, use the verbose option in the ValueIterationSolver")
    end
...
function gauss_seidel(solver::ParallelValueIterationMacroSolver, mdp::Union{MDP, POMDP})

    # state order is not used here!
    ...
    util = SharedArray{Float64}(ns, init = S -> S[localindexes(S)] = init_util[localindexes(S)])
    qmat  = SharedArray{Float64}((ns, na), init = S -> S[Base.localindexes(S)] = init_qmat[localindexes(S)])
    pol = SharedArray{Int64}(ns, init = S -> S[Base.localindexes(S)] = init_pol[localindexes(S)])
    residual = SharedArray{Float64}(1, init = S -> S[Base.localindexes(S)] = 0.)
...
@POMDP_require solve(solver::ParallelValueIterationMacroSolver, mdp::Union{MDP,POMDP}) begin
